...
SMALL_TEST_FILE = os.path.join(SCRIPT_DIR, "", "small_test.csv")
COLLINEAR_TEST_FILE = os.path.join(SCRIPT_DIR, "", "collinear_data.csv")
...
def load_csv_data(filename):
    ...
    X = np.array(
        [[v for k, v in datum.items() if k.lower().startswith("x")] for datum in data]
    )
...
def test_lambda_effect_on_sparsity(small_test_data):
    ...
    assert (
        high_reg_zeros >= low_reg_zeros
    ), "Higher lambda should result in more zero coefficients."
...
def test_model_stability_on_repeated_runs(small_test_data):
    ...
    assert np.allclose(
        results_1.coef_, results_2.coef_
    ), "Model coefficients should be consistent across runs."
...
def test_model_with_all_zero_features():
    ...
    assert np.all(
        results.coef_ == 0
    ), "Model should output zero coefficients for zero-valued features."
...
def test_model_with_high_dimensional_data():
    ...
    results = model.fit(X, y)
    assert (
        results.coef_.shape[0] == 100
    ), "Model should handle high-dimensional data properly."
...
def test_random_seed_impact(small_test_data):
    ...
    assert np.allclose(
        results_1.coef_, results_2.coef_
    ), "Results should be identical since model is deterministic."
...
def test_model_output_consistency_on_same_input(small_test_data):
    ...
    assert np.allclose(
        preds_1, preds_2
    ), "Predictions should be consistent on repeated runs."
